...
namespace ct.console.tests
{
    public class CtArgsExtensionsTests
    {
        ...
        [Fact]
        public void GetEncryptionKey_ShouldReturnKeyFromFile()
        {
            // Arrange
            const string encryptionKeyFilePath = "enc.key";
            const string encryptionKey = "MySecretKey";
            File.WriteAllText(encryptionKeyFilePath, encryptionKey);

            try
            {
                // Act
                var result = CtArgsExtensions.GetEncryptionKey();

                // Assert
                Assert.Equal(encryptionKey, result);
            }
            finally
            {
                // Cleanup
        ...
        [Fact]
        public void GetEncryptionKey_ShouldThrowFileNotFoundExceptionWhenKeyFileMissing()
        {
            // Arrange
            const string encryptionKeyFilePath = "enc.key";
            if (File.Exists(encryptionKeyFilePath)) File.Delete(encryptionKeyFilePath);

            // Act + Assert
            var exception = Assert.Throws<FileNotFoundException>(() => CtArgsExtensions.GetEncryptionKey());
            Assert.StartsWith("Encryption key file not found", exception.Message);
        }

        [Fact]
        public void GetEncryptionKey_ShouldThrowArgumentExceptionWhenKeyFileIsEmpty()
        {
            // Arrange
            const string encryptionKeyFilePath = "enc.key";
            File.WriteAllText(encryptionKeyFilePath, "");

            try
            {
                // Act + Assert
                var exception = Assert.Throws<ArgumentException>(() => CtArgsExtensions.GetEncryptionKey());
                Assert.Equal("Encryption key file is empty.", exception.Message);
            }
            finally
            {
                // Cleanup
